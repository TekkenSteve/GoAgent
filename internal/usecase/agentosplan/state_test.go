...
func TestStateRetryScheduledKeepsAttemptsAndClearsActiveRun(t *testing.T) {
	...
	requireStateNodeAttempt(t, &running, 1, "run-backend")
	...
	requireStateNodeAttempt(t, &second, 2, "run-2")
}

func requireStateNodeAttempt(t *testing.T, node *agentos.PlanNodeStatus, attempts int32, runID string) {
	t.Helper()

	if node.Attempts != attempts || node.RunID != runID {
		t.Fatalf("node = %#v, want attempts=%d runID=%q", node, attempts, runID)
	}
}
