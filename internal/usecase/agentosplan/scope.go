...
func ValidatePlanRef(ref agentos.PlanRef) error {
	if ref.PlanID == "" {
		return fmt.Errorf("%w: plan id is required", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidateRunPlanScope(spec *agentos.RunPlanSpec) error {
	...
	if spec.AccountID == "" {
		return fmt.Errorf("%w: account id is required", agentoscore.ErrInvalidRunPlan)
	}

	if spec.ProjectID == "" {
		return fmt.Errorf("%w: project id is required", agentoscore.ErrInvalidRunPlan)
	}
...
func ValidatePlanStreamScope(scope *agentos.PlanStreamScope) error {
	if scope == nil {
		return fmt.Errorf("%w: plan stream scope is required", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidatePlanEventScope(scope *agentos.PlanEventScope) error {
	if scope == nil {
		return fmt.Errorf("%w: plan event scope is required", agentoscore.ErrInvalidPlanScope)
	}
	...
	if scope.Limit < 0 {
		return fmt.Errorf("%w: event limit must be non-negative", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidatePlanDebugTraceScope(scope *agentos.PlanDebugTraceScope) error {
	if scope == nil {
		return fmt.Errorf("%w: plan debug trace scope is required", agentoscore.ErrInvalidPlanScope)
	}
	...
	if scope.Limit < 0 {
		return fmt.Errorf("%w: debug trace limit must be non-negative", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidatePlanAuditScope(scope *agentos.PlanAuditScope) error {
	if scope == nil {
		return fmt.Errorf("%w: plan audit scope is required", agentoscore.ErrInvalidPlanScope)
	}
	...
	if scope.Limit < 0 {
		return fmt.Errorf("%w: audit limit must be non-negative", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidatePlanArtifactScope(scope *agentos.PlanArtifactScope) error {
	if scope == nil {
		return fmt.Errorf("%w: plan artifact scope is required", agentoscore.ErrInvalidPlanScope)
	}
	...
	if scope.Limit < 0 {
		return fmt.Errorf("%w: artifact limit must be non-negative", agentoscore.ErrInvalidPlanScope)
	}
...
func ValidatePlanTenantAccess(ref agentos.PlanRef, spec *agentos.RunPlanSpec) error {
	...
	if spec == nil {
		return fmt.Errorf("%w: %s", agentoscore.ErrPlanRouteNotFound, ref.PlanID)
	}
...
func ScopePlanEventToSpec(event *agentos.PlanEvent, spec *agentos.RunPlanSpec) (agentos.PlanEvent, error) {
	...
	if scoped.PlanID != spec.PlanID {
		return agentos.PlanEvent{}, fmt.Errorf("%w: event plan %q does not match stored plan %q", agentoscore.ErrInvalidPlanEvent, scoped.PlanID, spec.PlanID)
	}

	if scoped.AccountID != "" && scoped.AccountID != spec.AccountID {
		return agentos.PlanEvent{}, fmt.Errorf("%w: event account %q does not match stored plan account %q", agentoscore.ErrInvalidPlanEvent, scoped.AccountID, spec.AccountID)
	}

	if scoped.ProjectID != "" && scoped.ProjectID != spec.ProjectID {
		return agentos.PlanEvent{}, fmt.Errorf("%w: event project %q does not match stored plan project %q", agentoscore.ErrInvalidPlanEvent, scoped.ProjectID, spec.ProjectID)
	}
