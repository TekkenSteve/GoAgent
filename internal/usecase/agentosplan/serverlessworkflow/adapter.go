...
func (a Adapter) Export(ctx context.Context, spec agentos.RunPlanSpec) (Workflow, error) {
	...
	if err != nil {
		return Workflow{}, fmt.Errorf("%w: encode run plan extension: %s", agentos.ErrInvalidRunPlan, err)
	}
	...
	for _, node := range spec.Nodes {
		...
		if err != nil {
			return Workflow{}, fmt.Errorf("%w: encode node extension: %s", agentos.ErrInvalidRunPlan, err)
		}
