...
var errTestDuplicateStreamID = errors.New("duplicate stream id")
...
func TestRedisPlanEventStreamPublishIsIdempotent(t *testing.T) {
	...
	stream := newRedisPlanEventStream(fake, nil)
	event := testPlanEvent("evt-1")
	fake.store(planEventStreamKey(testPlanRef()), &event)
...
func TestRedisPlanEventStreamRejectsSequenceCollision(t *testing.T) {
	...
	fake := newFakePlanEventRedis()
	stream := newRedisPlanEventStream(fake, nil)
	existing := testPlanEvent("evt-existing")
	fake.store(planEventStreamKey(testPlanRef()), &existing)
...
func (r *fakePlanEventRedis) StreamAddWithID(_ context.Context, stream, id string, values map[string]any, _ int) (string, error) {
	r.addCalls++
	...
	if _, exists := r.entries[stream][id]; exists {
		return "", errTestDuplicateStreamID
	}
...
func (r *fakePlanEventRedis) StreamRange(_ context.Context, stream, start, _ string, _ int) ([]goredis.XMessage, error) {
...
func (r *fakePlanEventRedis) store(stream string, event *agentos.PlanEvent) {
	...
	id := planEventEntryID(event.Sequence)
...
func (r *fakePlanEventRedis) entry(stream, id string) (goredis.XMessage, bool) {
